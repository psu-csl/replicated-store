...
// using InstanceState::INPROGRESS;
...
TEST_F(LogTest, Insert) {
  ...
  EXPECT_EQ(PUT, log[index].command_.type_);
...
TEST_F(LogTest, InsertUpdateInProgress) {
  ...
  EXPECT_EQ(DEL, log[index].command_.type_);
}
...
TEST_F(LogTest, Append) {
  ...
  log_.Append(MakeInstance(0, log_.AdvanceLastIndex()));
  EXPECT_EQ(1, log_.at(1)->index_);
  EXPECT_EQ(2, log_.at(2)->index_);
}

TEST_F(LogTest, AppendWithGap) {
  auto index = 42;
  log_.Append(MakeInstance(0, index));
  EXPECT_EQ(index, log_.at(index)->index_);
...
TEST_F(LogTest, AppendHighBallotOverride) {
  ...
  EXPECT_EQ(DEL, log_.at(index)->command_.type_);
}
...
TEST_F(LogTest, AppendLowBallotNoEffect) {
  ...
  EXPECT_EQ(PUT, log_.at(index)->command_.type_);
}
