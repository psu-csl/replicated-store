- MultiPaxos
  ...
  - types/protobufs
    ...
    - prepare_state
      - num_rpcs_: number of RPC responses received
      - num_oks_: number of positive prepare responses
      - max_last_index_: highest index observed in instances received
  ...
  - members
    ...
    - rpc_peers_: an array of RPC endpoints to peers.
      initialized from the configuration file.
  ...
  - methods
    ...
    - become_follower(new_ballot: int64)
      ...
      # type). in both scenarios, it sets this peers ballot to its argument as 
      # long as the argument is higher than this peer ballot, and it checks to 
      # see if this peer just became a follower from the leader status,if so, it
    ...
    - run_prepare_phase(ballot: int64) -> optional pair{int64, map{int64 -> instance}
      ...
      if ballot > ballot_:
        ...
        state.num_oks_++
        state.log_ = log_.log()
        state.max_last_index_ = log_.last_index()
      else:
        return nullopt
      ...
      for peer in rpc_peers_:
       ...
       state.mu_.lock()
       # sleep while quorum is not reached and not all peers have responded; 
       # since this peer has already handles the prepare request before sending 
       # out RPC requests, our condition check is <= and not <.
       while state.num_oks <= num_peers_ / 2 &&
       ...
       if state.num_oks > num_peers_ / 2:
         return {state.max_last_index_, state.log_}
    ...
    - run_accept_phase(ballot: int64, index: int64, cmd: command, 
      ...
      # if the peer is no longer a leader, the function can stop waiting and
      # return earlier. even if this instance is accepted by majority peers, the
      # new leader will recover it and commit it eventually. The reason why
      # run_prepare_phase does not have the leader checking is because the peer
      # is never a leader in that case.
      state.mu_lock()
      while IsLeader(ballot_, id_) &&
