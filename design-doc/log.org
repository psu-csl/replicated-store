- Log
  ...
  # instance. Wake up happens only one way: the thread that commits wakes up the
  # executor thread to execute instances. Since executor thread may block on
  # calling the execute method of log, we need to wake up the executor when we
  # want to cleanly shut down the system. To this end, the log has a running
  # flag and a stop method that sets the running flag to false and signals the
  # condition variable upon which the executor is sleeping due to calling the
  # execute method. The execute method will return a nullopt after the log is
  # stopped and the executor method can exit upon receiving nullopt.
  ...
  - types/protobufs:
    ...
    - log_map_t: a map from int to Instance

    - log_vector_t: an array of Instances
  ...
  - public methods
    ...
    - execute(kv: KVStore) -> optional(client-id, result)
    ...
    - instances() -> Instance[]
      # description: return all instances in the log for a response to a prepare
      # message; since global_last_executed_ is the tail of the log and
      # last_index_ is the head of the log, we return all instances in-between.
      # preconditions:
      # postconditions:
      mu_.lock()
      instances = Instance[]
