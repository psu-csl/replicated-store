  ...
  - public methods
    ...
    - commit_until(leader_last_executed: int, ballot: int)
      ...
      * # we may receive a heartbeat before we receive the accept message;
        ...
        if (log_[i] == empty)
          break
      * # |ballot| will determine whether we will commit entries in the log.
        # we can have three cases with respect to |ballot| and the ballot of
        # instances in the log after last_executed_:
        #
        # case (1) they are equal: this is the common case. as a follower, we
        ...
        # case (2) |ballot| is larger: this is the uncommon case, and it can
        ...
        # will violate safety. in this scenario, we will get stuck here and
        # prevent global_last_executed_ from advancing, until a new leader is
        # elected, and it replays every instance since global_last_executed_ and
        # we discover the new commands that we missed during the partition.
        #
        # case (3) |ballot| is smaller: this is the impossible case because
        # commit_until is always called from the heartbeat_handler and that will
        # reject heartbeats with a lower ballot number than ours. we check this
        # with an assertion below.
...
- unit tests
  ...
  - append-with-gap
    - if we leave a gap in the log by appending an entry at last_index_ + m,
  ...
  - commit (death test)
    - committing with invalid index should fail.
  ...
  - commit-until
    - append entries at indexes 1, 2, 3, call commit-until with a different old
      ballot, and ensure that none of the instances were committed; call
      commit-until with the sa
