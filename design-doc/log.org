- Log
  ...
  - types:
    ...
    - Command: a key-value command. Fields of a command are
      - type_: enum { get, put, del }
      - key_: string
      - value_: string (valid only if type_ == put)

    - Log: a map from int to Instance

    - Instance: entry in the log. each instance starts in the in-progress state,
      and changes to the committed state once quorum has voted for it, and
      changes to executed state once the executor thread has its command
      executed. Fields of an instance are:
...
- unit tests
  ...
  - instances-for-prepare
    - append instances at indexes 1, 2, 3; call instances_for_prepare and ensure
      that all instances are returned; call commit_until with the same ballot
      number and leader_last_executed = 2, and then call trim_until with
      leader_global_last_executed = 2 to move global_last_executed_ to 2. call
      instances_for_prepare again, and ensure that only instance 3 is returned.
