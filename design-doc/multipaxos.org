...
- unit tests
  ...
  - run_commit_phase
    - check that run_commit_phase works correctly; specifically, it returns the
      ...
      - call run_commit_phase with global_last_executed = 0 2 expect 3 as the
        response

  - run_prepare_phase
    - check that run_prepare_phase works correctly; specifically, it returns
      null when it hasn't received responses from quorum, and it returns proper
      log when it has heard from the quorum.
