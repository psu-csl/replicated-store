...
- unit tests
  ...
  - send_prepares
    - ensure that send_prepares works correctly; specifically, it returns null
      when it hasn't received responses from quorum, and it returns proper log
      when it has heard from the quorum.
      - append to peer0, peer1 logs an instance at index 1
      - start peer0 rpc server
      - call send_prepares and expect null as the response
      - start peer1 rpc server
      - call send_prepares and expect a response with a log containing the
        instance
