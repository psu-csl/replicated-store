- MultiPaxos
  ...
  - types
    ...
    - prepare_rpc_response
      ...
      - log_: Instances of the peer since global_last_executed_ (valid only if
        type_ == ok)
    ...
    - prepare_handler(request: prepare_rpc_request)
      ...
      if request.ballot_ >= ballot_:
        ...
        return prepare_rpc_response{type_: ok,
                                    ballot_: N/A,
                                    log_: log_.instances_for_prepare()}
      # reject stale RPC requests
      return prepare_rpc_response{type_: reject, ballot_: N/A, log_: N/A}
