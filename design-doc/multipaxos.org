...
- unit tests
  ...
  - heartbeat_response_with_high_ballot_changes_leader_to_follower
    ...
    - call broadcast_heartbeat from peer0_ and expect it to become a follower
  ...
  - prepare_response_with_high_ballot_changes_leader_to_follower
    ...
    - send a heartbeat with peer2_'s ballot to peer1_ to establish peer2 as a
      leader in peer1 as well
    - call broadcast_prepare from peer0_ and expect it to become a follower
      assume peer2 as the leader
  ...
  - accept_response_with_high_ballot_changes_leader_to_follower
    ...
    - send a heartbeat with peer2_'s ballot to peer1_ to establish peer2 as a
      leader in peer1 as well
    - call broadcast_accept from peer0_ and expect it to become a follower
      assume peer2 as the leader

  - broadcast_heartbeat
    - ensure that broadcast_heartbeat works correctly; specifically, it returns
      the passed in global_last_executed when it hasn't heard from all of the
      peers, and it returns the new global_last_executed when it has heard from
      all of the peers.
      ...
      - call broadcast_heartbeat with global_last_executed = 0 and expect 0 as
        the response
      - start peer2 rpc server
      - call broadcast_heartbeat with global_last_executed = 0 and expect 1 as
        the response

  - broadcast_prepare
    - ensure that broadcast_prepare works correctly; specifically, it returns
      ...
      - call broadcast_prepare and expect null as the response
      - start peer1 rpc server
      - call broadcast_prepare and expect a response with a log containing the
        instance
