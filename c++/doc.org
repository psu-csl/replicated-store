...
- struct kv_result_t
  ...
  - fields:
    - type_: enum {ok, fail}
    - value_: value if type_ == ok, error message otherwise

- struct instance_t
  - synopsis: entry in the log.
  - fields:
    - round_: round of the instance (initialized during construction)
    - command_: command_t (initialized during construction)
    - index_: index of the command in the log. (initialized during construction)
    - state_: enum {in-progress, committed, executed} (initialized to in-progress)
...
- struct accept_result_t
  ...
  - fields:
    ...
    - value_: string (if type_ == ok then contains result of command; if type_
...
- next_round#() -> int
  ...
  round_ = ((round_ & 0xfffffff0) + 1) ## id_
  return round_
...
- agree_and_execute(command: command_t) -> accept_result_t
  ...
  if leader_ != null:
    return accept_result_t{type_: leader, leader_: leader_}
...
- accept(command: command_t, index: int) -> accept_result_t
  ...
  if leader_ != id_:
    return accept_result_t{type_: leader, result_: N/A, leader_: leader_}
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      while leader_ == id &&
            ...
            num_responses != peers_.size():
        cv.wait(mu)
      # if we are not a leader, then we should stop sending heartbeats
      if !leader_:
        break
      # we are still a leader; sleep before sending another round of heartbeats
...
- heartbeat_handler(message: heartbeat_rpc_request):
  ...
  if message.round >= round_:
    ...
    if leader_min_tail_ > min_tail_ || leader_tail_ > tail_:
      wake up execute_and_trim_thread
...
- execute_and_trim_thread():
  for (;;) {
    sleep until leader_min_tail_ or leader_tail_ has changed
