...
- next_round#() -> int
  round_ += 256  # increment the round portion by 1
  round_ = (round_ & 0xffffff00) | id_ # combine our id with the round number
  return round_
...
- someone_else_is_leader() -> bool
  return leader() != id_ && leader() < max_num_peers_
...
- accept(command: command_t, index: int) -> accept_result_t
  ...
  if ok_responses.size() > peers_.size()/2: # we have quorum
    ...
    lock(tail_mu_)
    assert(index > tail_)
    while (index != tail_+1 && i_am_leader())
