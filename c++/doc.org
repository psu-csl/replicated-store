...
- struct instance_t
  ...
  - fields:
    - proposal_num: the proposal number
    - command: command_t
    - index: index of the command in the log.
    - state: enum {in-progress, committed}
  - init:
    - proposal_num: provided during construction
...
- proposal_num_
  ...
  - description: also known as ballot or term, this number represents the
    ...
    next value, proposal_num_ is incremented by 2^n where n is the number of
    bits used for storing the id_ at the lower bits. for example, if we use the
    lower 4 bits to store id_, then the next proposal_num_ is obtained by
    proposal_num_ += 2^4.
  - reading threads:
    - TBD
  - writing threads:
...
- accept(command, index):
  log_[index] = instance{command, index, proposal#(), in-progress}
  num_responses = 0
  num_ok_responses = 0
  cv, mu
  for each peer P {
    run closure in a separate thread {
      response = P.acceptRPC(command, index, proposal#())
      lock(mu)
      ++num_responses
      if response is ok:
        ++num_ok_responses
...
- accept_handler(message):
  # common case
  if message.proposal_num == proposal_num_:
    log_[message.index] = instance{message.command, message.index, message.proposal_num, in-progress}
    return ok
  # stale message
  if message.proposal_num < proposal_num_:
    return reject(proposal_num_)
