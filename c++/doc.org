...
- phase1 thread: each paxos object runs a phase1 (or election) thread. this
  ...
  leader is still alive. whenever it detects a dead leader, it runs phase1 to
  become a leader. at the end of the phase1 either this object or someone else
...
- request threads: in a main thread, agree_and_execute() calls phase2() which
  launches RPCs to peers to get the quorum vote; in phase1 thread, phase1()
...
- peers_
  ...
  - reading threads:
    - main threads: phase2() reads peers_ when sending RPCs.
    - heartbeat thread: reads peers_ when sending heartbeat messages.
    - phase1 thread: phase1() reads peers_ when sending RPCs.
  - writing threads:
    - none
...
- log_
  ...
  - init: empty
  - description: abstractly, it is a journal with a head and tail, where
    ...
    the tail_ (see below) when all peers have told us their tail. so, if only
    one of the peers becomes partitioned, it will prevent us moving the tail,
    and we will quickly fill the circular buffer and stall the progress because
    only one of the peers is temporarily partitioned. therefore, we are going to
    use a hashmap from index to instance for this data structure. without going
    into details, asymptotically, it should act as a resizeable circular buffer.
