...
- struct instance_t
  ...
  - fields:
    ...
    - state_: enum {in-progress, committed}
  - init:
...
- message accept_rpc_response
  - synopsis: acceptRPC response protobuf
  - fields:
...
- message prepare_rpc_request
  - synopsis: prepareRPC request protobuf
  - fields:
    - tail_: int
    - round_: int

- message prepare_rpc_response
  - synopsis: prepareRPC response protobuf
  - fields:
    - type_: enum {ok, reject, timeout}
    - round_: round of the remote peer; valid only if type_ == reject
    - log_: instances since tail_ in the corresponding prepareRPC request
...
- max_num_peers_:
  ...
  - description: it is also used as a delta for incrementing round_ (see below)
    ...
    in round_ store the id_ of the peer. we initialize round_ to the id_ of the
    peer and increment it by max_num_peers_, which keeps id_ bits intact and
    increments the higher bits. this allows us to obtain unique round_ numbers.
...
- next_round#() -> int
  round_ += max_num_peers_
  return round_

- agree_and_execute(command: command_t) -> result_t
  if leader_ == id_:
    if ready_:
      return accept(command, ++head_)
    return result_t{type_: retry, result_: N/A, leader_: N/A}
  # someone else is leader
  if leader_ != null:
    return result_t{type_: leader, leader_: leader_}
  # no one is leader---an election is in progress; interestingly, this path will
  # be taken just once during the lifetime of the object because we never reset
  # leader_ to null after the first election. we only reset it to the id of
  # another peer once, whenever we receive a higher-numbered ballot than ours.
  return result_t{type_: retry, result_: N/A, leader_: N/A}

- accept(command: command_t, index: int) -> result_t
  num_responses = 0
  ok_responses = vector
  cv, mu
  ...
  for each peer p {
    run closure in a separate thread {
      ...
      else if respose.type_ == reject:
        leader_ = response.round_ && (max_num_peers_ - 1)
  ...
  if ok_responses.size() > peers_.size()/2: # we have quorum
    ...
    tail_cv_.notify_one()
    if result.ok:
      return result_t{type_: ok, result_: result.value, leader_: N/A}
    else
      return result_t{type_: fail, result_: result.error, leader_: N/A}
  if leader_ != id_:
    return result_t{type_: leader, result_: N/A, leader_: leader_}
...
- accept_handler(message: accept_rpc_request):
  ...
  if instance.round > round_:
    leader_ = message.round_ && (max_num_peers_ - 1)
    log_[message.index_] = i
...
- prepare() -> prepare_result_t:
  ...
  if ok_responses.size() > peers_.size()/2: # we have quorum
    ...
    return prepare_result_t{type_: ok, round_: round_, log_: log}
  if leader_ != id_:
    return prepare_result_t{type_: reject, round_: }
  # multiple timeout responses
  return retry

- prepare_handler(tail, round):
  # common case for phase1
  if round >= round_:
    leader_ = round && (max_num_peers_ - 1)
    return (ok, log_[tail+1:])
  # stale messages
...
- prepare_thread():
  for (;;) {
    ...
    for (;;) {
      ...
      do {
        response = prepare()
      } while response.type_ != timeout
      if response.type_ == reject:
        continue
      # response type is ok
