...
- struct instance_t
  ...
  - init:
    - round_: provided during construction
    - command_: provided during construction
    - index_: provided during construction
    - state_: in-progress
...
- accept(command: command_t, index: int) -> accept_result_t
  ...
  if ok_responses.size() > peers_.size()/2: # we have quorum
    ...
    tail_cv_.notify_one()
    if result.ok:
      return accept_result_t{type_: ok, result_: result.value, leader_: N/A}
    else
      return accept_result_t{type_: fail, result_: result.error, leader_: N/A}
  if leader_ != id_:
