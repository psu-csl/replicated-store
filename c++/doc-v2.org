...
- Log
  ...
  - methods
    ...
    - append(instance_: instance_t)
      ...
      # append is a no-op if it receives an instance for a slot in the log that
      # is already committed, executed, and pruned. it is possible to receive
      # such an instance, for example, if
      #
      ...
      # append is a no-op if it receives an instance for a slot in the log that
      # is already a committed or executed; furthermore, in a situation like
      # this, the instance_'s command must match the command in the log.
      if log_[i] != empty && log_[i].status == (committed or executed)
        # do nothing---just assert and return
        if instance_.status_ == (committed or executed)
          assert(log_[i].cmd_ == instance_.cmd_)
        return

      # otherwise, if we don't have instance_ or if the instance_ has a higher
      # round number, then we just put it in our log.
      if log_[i] == empty || log_[i].round_ < instance_.round_
        log_[i] = instance
        log_[i].status = in-progress

      # the remaining case is log_[i] is not empty and log_[i].status ==
      # in-progress and
      #
      # (1) instance_.round < log_[i].round_
      # or
      # (2) instance_.round == log_[i].round_
      #
      # we call append in two places: (a) in the accept_handler(), when we are a
      # follower and we receive an accept message, and (b) in log_.merge(), when
      # we are a leader and we have received logs from the quorum and we want to
      # merge the received logs.
      #
      # The case (1) cannot happen in (a) because the accept_handler() ignores
      # requests with round number lower than ours; but case (1) can happen in
      # (b) because we may have received a log with stale entries, in which case
      # we ignore such an entry.
      #
      # The case (2) can happen in (a) if somehow we receive the same accept
      # message twice, in which case the assertion below must hold; case (2) can
      # also happen in (b) because we may have received logs from followers that
      # had the same instance in their log in in-progress state and they
      # responded to us, in which case, again, the assertion below must hold.
      if instance_.round == log_[i].round_
        assert(instance_.cmd_ == log_[i].cmd_)
