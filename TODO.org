...
* todo refactoring
** go over the code base and remove redundant code (DummyPaxos.java)
** fix function parameter orders (e.g. go NewMultipaxos)
** fix the MultiPaxos constructor field initialization order (go, java)
** make sure that variable declaration order and function definition order in all objects is the same in the design doc
** java: add port field to multipaxos
** java: move rpc server creation to startrpcserver
** go: paxos -> multipaxos
** java: in start() function fix order
...
** fix the concurrency bug in go and c++ in stopXXXthread functions
